...
def plot_movements(genetic_code: List[int] = None, grid: Grid = None, grid_from_csv: bool = False, **kwargs) -> None:
    ...
    for _ in range(iterations):
        ...
        print('\nLegend:')
        print(f'\t {MovementRepr.current_position_repr[MovementRepr.can_repr[0]]} \t- robot on an empty square')
        print(f'\t {MovementRepr.current_position_repr[MovementRepr.can_repr[1]]} \t- robot on a square with a can')
        print(f'\t {MovementRepr.can_repr[0]} \t- empty square')
        print(f'\t {MovementRepr.can_repr[1]} \t- can')
